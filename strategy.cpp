...
Cards Strategy::firstPlay()
{
    ...
    if(hasPlane)
    {
        ...
        for(Card::CardPoint point = Card::Point_3; point <= Card::Point_10; point = Card::CardPoint(point + 1))
        {
    ...
    if(hasTriple)
    {
        if(PlayHand(seqTripleArray[0]).getCardPoint() < Card::Point_A)
        {
            for(Card::CardPoint point = Card::Point_3; point <= Card::Point_A; point = Card::CardPoint(point+1))
            {
    ...
    if(nextPlayer->getCards().cardCount() == 1 && m_player->getRole() != nextPlayer->getRole())
    {
        for(Card::CardPoint point = Card::CardPoint(Card::Point_End-1);
             point >= Card::Point_3; point = Card::CardPoint(point-1))
        {
            int pointCount = backup.pointCount(point);
            if(pointCount == 1)
            {
    ...
    {
        for(Card::CardPoint point =  Card::Point_3;
...
Cards Strategy::getGreaterCards(PlayHand type)
{
    ...
    if(pendPlayer != nullptr && pendPlayer->getRole() != m_player->getRole() && pendPlayer->getCards().cardCount() <= 3)
    {
        ...
        // 搜索当前玩家手中有没有王炸
        Cards sj = findSamePointCards(Card::Point_SJ, 1);
        Cards bj = findSamePointCards(Card::Point_BJ, 1);
...
bool Strategy::whetherToBeat(Cards &cs)
{
    ...
    if(m_player->getRole() == pendPlayer->getRole())
    {
        ...
        if(basePoint == Card::Point_2 || basePoint == Card::Point_SJ || basePoint == Card::Point_BJ)
        {
            return false;
        }
    }
    else
    {
        ...
        if((myHand.getHandType() == PlayHand::Hand_Triple_Single || myHand.getHandType() == PlayHand::Hand_Triple_Pair)
            && myHand.getCardPoint() == Card::Point_2)
        {
            return false;
        }

        // 如果cs是对2, 并且出牌玩家手中的牌数量大于等于10 && 自己的牌的数量大于等于5, 暂时放弃出牌
        if(myHand.getHandType() == PlayHand::Hand_Pair && myHand.getCardPoint() == Card::Point_2
...
Cards Strategy::findSamePointCards(Card::CardPoint point, int count)
{
    ...
    // 大小王
    if(point == Card::Point_SJ || point == Card::Point_BJ)
    {
        if(count > 1)
        {
            return Cards();
...
QVector<Cards> Strategy::findCardsByCount(int count)
{
    ...
    for(Card::CardPoint point = Card::Point_3; point < Card::Point_End; point = (Card::CardPoint)(point+1))
    {
...
QVector<Cards> Strategy::findCardType(PlayHand hand, bool beat)
{
    ...
    // 确定起始点数
    Card::CardPoint beginPoint = beat ? Card::CardPoint(point + 1) : Card::Point_3;
...
QVector<Cards> Strategy::getPlane(Card::CardPoint begin)
{
    ...
    for(Card::CardPoint point = begin; point <= Card::Point_K; point = (Card::CardPoint)(point+1))
    {
        // 根据点数和数量进行搜索
    ...
    if(info.beat)
    {
        ...
        for(Card::CardPoint point = info.begin; point <= info.end; point = (Card::CardPoint)(point+1))
        {
            ...
            for(int i=0; i<info.extra; ++i)
            {
                ...
                if(cards.isEmpty() || (point + info.extra >= Card::Point_2))
                {
                    found = false;
                    seqCards.clear();
                    break;
