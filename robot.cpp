...
void Robot::thinkCallLord()
{
    ...
    */
    ...
    weight += st.getRangeCards(Card::Point_SJ, Card::Point_BJ).cardCount() * 6;
    ...
    weight += m_cards.pointCount(Card::Point_2) * 3;

    Cards tmp = m_cards;
    tmp.remove(optSeq);
    tmp.remove(bombs);
    Cards card2 = st.getRangeCards(Card::Point_2, Card::Point_2);
    tmp.remove(card2);
